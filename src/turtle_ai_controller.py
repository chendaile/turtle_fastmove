...
class turtle_node(Node):
    def __init__(self, continue_bool, route_name):
        ...
        self.cmd_sender = self.create_publisher(Twist, "/turtle1/cmd_vel", 10)
    ...
    def get_best_cmd(self, distance, angle_diff):
        ...
        if abs(angle_diff) > self.param.params["angle_threshold"]:
            # 需要转向
            w = (
                self.param.params["turn_speed"]
                if angle_diff > 0
                else -self.param.params["turn_speed"]
            )
            v = max(
                self.param.params["min_turn_speed"],
                self.param.params["max_speed"]
                - abs(angle_diff) * self.param.params["speed_angle_factor"],
            )
        else:
            # 可以直走
            ...
            v = min(
                self.param.params["max_speed"],
                distance * self.param.params["speed_distance_factor"],
            )
    ...
    def mainloop(self):
        ...
        if (
            self.total_node > 0
            and self.total_node % self.len_route == 0
            and not self.havePrint
        ):
            self.havePrint = True
        ...
        if (
            hasattr(self, "lap_start_time")
            and time.time() - self.lap_start_time > 30
            and not self.haveFix
        ):
            self.duty_index = (self.duty_index + 1) % len(self.duty)
            self.haveFix = True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Please select the required turtle running parameters"
    )
    parser.add_argument(
        "--contin",
        "-u",
        action="store_true",
        default=False,
        help="Whether to continue from a given params",
    )
    parser.add_argument(
        "--route",
        "-r",
